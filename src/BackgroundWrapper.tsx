...
function BackgroundWrapper({ children }: BackgroundWrapperProps) {
  ...
  const eligibleImages = getEligibleImages(viewport.w, viewport.h);
  ...
  useEffect(() => {
    ...
    fetchImage(nextRawUrl, controller.signal)
      .then((objectUrl) => {
        if (cancelled) {
